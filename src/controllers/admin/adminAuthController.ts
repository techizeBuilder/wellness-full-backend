...
const adminLogin = asyncHandler(async (req, res) => {
  ...
  const secret = process.env.JWT_SECRET_ADMIN || ENV.JWT_SECRET;
  if (!secret) {
    return res
      .status(500)
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  const matched = await (admin as any).matchPassword(oldPassword);
  if (!matched)
    return res
      .status(401)
  ...
  try {
    ...
    if (emailResult.success) {
      ...
      return res.status(500).json({
        success: false,
        message:
          "Failed to send reset email after multiple attempts. Please try again later.",
