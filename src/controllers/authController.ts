...
const normalizeProfileImagePath = (
...
// Helper function to detect whether an email belongs to a user or expert
const detectUserType = async (email: string): Promise<"user" | "expert"> => {
  const user = await userRepository.findByEmail(email);
  if (user) return "user";
  const expert = await expertRepository.findByEmail(email);
  if (expert) return "expert";
  // Default to 'user' so the service throws its standard 'User not found' error
  return "user";
};
  ...
  async (req: Request, res: Response) => {
    ...
    if (currentUser.constructor.modelName === "Expert") {
      ...
      if (expertObj.profileImage) {
        expertObj.profileImage = getFileUrl(
          normalizeProfileImagePath(expertObj.profileImage),
          "profiles",
        );
      }
    ...
    if (userObj.profileImage) {
      userObj.profileImage = getFileUrl(
        normalizeProfileImagePath(userObj.profileImage),
        "profiles",
      );
    }
...
export const forgotPassword = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const userType = await detectUserType(email);
...
// @access  Public
export const verifyPasswordResetOTPUnauthenticated = asyncHandler(
  async (req: Request, res: Response) => {
    const { email, otp } = req.body;
    const userType = await detectUserType(email);
    const result = await authService.verifyPasswordResetOTPUnauthenticated(
      email,
      otp,
      userType,
    );
...
// @access  Public
export const resetPasswordWithOTPUnauthenticated = asyncHandler(
  async (req: Request, res: Response) => {
    const { email, otp, password } = req.body;
    const userType = await detectUserType(email);
    const result = await authService.resetPasswordWithOTPUnauthenticated(
      email,
      otp,
      password,
      userType,
  ...
  async (req: Request, res: Response) => {
    ...
    if ((req as any).file) {
      ...
      if (currentUser.profileImage) {
        const oldImagePath = normalizeProfileImagePath(
          currentUser.profileImage,
        );
        if (oldImagePath) {
          deleteFile(
            path.join(__dirname, "..", "uploads", "profiles", oldImagePath),
          );
        }
      }
