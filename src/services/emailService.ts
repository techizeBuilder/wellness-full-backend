...
export const sendEmail = async (
  ...
  try {
    logger.info(
      `Sending email to: ${options.email}, Subject: ${options.subject} (Attempt ${retryCount + 1})`,
    );

    const mailOptions = {
      from: `"VedaNova" <${ENV.EMAIL_FROM}>`,
      to: options.email,
    ...
    logger.error(
      `Email sending failed after ${duration}ms (Attempt ${retryCount + 1})`,
      error,
    );
...
export const sendOTPEmail = async (
  ...
  if (type === "password_reset") {
    subject = "Password Reset OTP - VedaNova";
    purposeText =
      "You have requested to reset your password. Please use the OTP code below to verify your identity, then click the reset button to set your new password:";
    headerTitle = "Password Reset Request";
  } else {
    subject = "Your OTP Code - VedaNova";
    purposeText =
      "You have requested a One-Time Password (OTP) for verification. Please use the code below:";
  ...
  const html = `
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                <p>&copy; ${new Date().getFullYear()} VedaNova. All rights reserved.</p>
            </div>
        </div>
    </body>
    </html>
...
export const sendPasswordResetEmail = async (
  ...
  const subject = "Password Reset Request - VedaNova";
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
...
export const sendWelcomeEmail = async (
  ...
  const subject = `Welcome to VedaNova, ${firstName}!`;
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Welcome to VedaNova</title>
        <style>
            body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
            .container { max-width: 600px; margin: 0 auto; padding: 20px; }
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>Welcome to VedaNova!</h1>
            </div>
            <div class="content">
                <h2>Hello ${firstName}!</h2>
                <p>Welcome to VedaNova! We're excited to have you join our community.</p>
...
export const sendExpertApprovalEmail = async (
  email: string,
  firstName: string,
...
export const sendBookingConfirmationEmail = async ({
  ...
  const ctaUrl =
    role === "user" ? `vedanova://sessions` : `vedanova://expert-appointments`;
  ...
  const plainTextLines = [
    ...
    planName
      ? `Plan: ${planName}${planSessionNumber && planTotalSessions ? ` (Session ${planSessionNumber} of ${planTotalSessions})` : ""}`
      : "",
...
export const sendBookingStatusUpdateEmail = async ({
  ...
  const ctaUrl = `vedanova://sessions`;
