...
export const expertRegisterSchema = Joi.object({
  firstName: Joi.string()
    ...
    .messages({
      ...
      "string.max": "Full Name cannot exceed 50 characters.",
      "string.pattern.base":
        "Full Name can only contain letters, spaces, and underscores.",
    }),
  lastName: Joi.string()
    .trim()
    .min(2)
  ...
  experience: Joi.number().integer().min(0).max(50).optional().messages({
    "number.base":
      "Please enter a valid non-negative whole number for years of experience.",
  ...
  bio: Joi.string()
    ...
    .max(1000)
    .optional()
    .messages({ "string.max": "Bio cannot exceed 1000 characters." }),
  ...
  qualifications: Joi.array()
    ...
    .optional()
    .messages({ "array.min": "At least one qualification is required." }),
...
export const updateProfileSchema = Joi.object({
  ...
  bloodPressure: Joi.string()
    ...
    .messages({
      "string.pattern.base":
        "Blood pressure must be in the format S/D (e.g., 120/80)",
    }),
});
...
export const updateExpertProfileSchema = Joi.object({
  ...
  sessionType: Joi.array()
    .items(Joi.string().valid("one-on-one", "one-to-many"))
    .optional(),
...
export const validate = (schema: ObjectSchema) => {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    if (error) {
      ...
      return res
        .status(400)
        .json({ success: false, message, errors, type: "validation_error" });
    }
    next();
  };
};
